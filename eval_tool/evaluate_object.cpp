...
const string GT_DIR         = "data/kitti/object/training/label_2";
const string RESULT_DIR     = "models_stereo/result";
...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  for (int v = 0; v < 3; ++v)
      for (int i = 0; i < vals[v].size(); i = i + 4)
          sum[v] += vals[v][i];
  sprintf(ap_result,"%s AP: %f %f %f\n", file_name.c_str(), sum[0] / 11 * 100, sum[1] / 11 * 100, sum[2] / 11 * 100);
  printf("%s", ap_result);
  fp = fopen((dir_name + "/../AP_result.txt").c_str(), "a");
  fprintf(fp, "%s", ap_result);
  fclose(fp); 
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc != 1) {
    cout << "Usage: ./evaluate_object" << endl;
    return 1;
  }

  // run evaluation
  if (eval()) {
    cout << "Your evaluation results are available at models_stereo/result folder." << endl;
  } else {
